...
def mostrar_todos_los_meses(request):
    ...
    if tipo_contraparte is not None:
        meses_y_registros = DatosLimpios.objects.values('ano', 'mes').annotate(cantidad_registros=Count('mes'))
        meses_y_nombres = []

        for mes_registro in meses_y_registros:
            objetos = DatosLimpios.objects.filter(ano=mes_registro['ano'], mes=mes_registro['mes'])
...
def mostrar_todos_los_anios(request):
    tipo_contraparte = request.session.get('tipo_contraparte')
    if tipo_contraparte is not None:
        años_y_registros = DatosLimpios.objects.values('ano').annotate(cantidad_registros=Count('ano'))
...
def descargar_archivo(request, ano, mes):
    try:
        datos_limpios = DatosLimpios.objects.filter(mes=mes).first()
